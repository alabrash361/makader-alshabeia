...
document.getElementById('scroll-btn').addEventListener('click', function () {
    window.scrollBy({
        top: 650, // القيمة التي تريد التمرير بها
...
const textContainer = document.querySelector(".centerh1");
const phrases = ["Innovative", "Artistic", "Inspired", "Visionary"];
let phraseIndex = 0;
let letterIndex = 0;
...
function typeEffect() {
  ...
  textContainer.innerHTML = `<h1> Creativa Is ${displayText}</h1>`;
  if (!isDeleting && letterIndex < currentPhrase.length) {
    ...
    setTimeout(typeEffect, 850); // Pause before switching
  }
}
typeEffect();